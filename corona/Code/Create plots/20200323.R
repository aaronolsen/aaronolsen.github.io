...
run <- function(){
	...
	start_cases <- 20	# Number of cases when to align curves
	...
	for(state in states_unique){
		...
		if(sum(which_over_min) >= min_num_val_lm){
			...
			lm_fit_log_sfp <- lm(y ~ x, data=dlist)
			start_day_int <- -(lm_fit_log_sfp$coefficients['(Intercept)'] - log(start_cases, base=log_base)) / (lm_fit_log_sfp$coefficients['x'])
			...
			if(FALSE){
				pdf(paste0('~/Documents/Outreach/Coronavirus analysis/Plots/align curves.pdf'), height=7, width=7)

				plot(dlist$x, dlist$y, type='n')

				points(dlist$x, dlist$y)
				points(dlist$x, dlist$y, type='l')
			
				abline(lm_fit_log_sfp)
				abline(v=start_date, lty=2)
				abline(h=log(start_cases, base=log_base), lty=3)
	...
	w_ratio <- 1.4
	mtext_cex <- 0.8
	pdf(paste0('~/Documents/Outreach/Coronavirus analysis/Plots/positive vs ', x_val, ' ranked_by=', ranked_by, '/', file_date, ' positive vs ', x_val, 
	...
	for(y_val in c('positive', 'positive_log')[1:2]){
		...
		if(y_val == 'positive_log' && x_val %in% c('Days_SFP', 'Days_SSD')) x_range <- c(0, x_range[2])
		if(y_val == 'positive_log' && x_val == 'Days_SSD') y_range[1] <- log(start_cases, base=log_base)
		if(y_val == 'positive') x_range[1] <- 5
		...
		if(y_val == 'positive') mtext(text='Number of reported positive COVID-19 tests', side=2, line=2.5, cex=mtext_cex)
		if(y_val == 'positive_log') mtext(text='Number of reported positive COVID-19 tests, log-transformed', side=2, line=2.5, cex=mtext_cex)
