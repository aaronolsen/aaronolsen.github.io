...
run <- function(){
	...
	# Remove rows with NA nfold
	testing_stats <- testing_stats[!is.na(testing_stats[, 'nfold_pday']), ]
	...
	rank_order <- list(
		'slope'=rownames(log_rate_lm_mat_nna)[order(log_rate_lm_mat_nna[, 'slope'], decreasing=TRUE)],
...
#		'intercept'=rownames(log_sfp_lm_mat_nna)[order(log_sfp_lm_mat_nna[, 'intercept'])],
	...
	legend_labels <- paste0(rank_order[[ranked_by]], ' (#', length(rank_order[[ranked_by]]):1, ')')
	#print(legend_labels)

	# Set states to plot
	#mid_ranks <- round(seq(length(rank_order[[ranked_by]])-mar_rank[1], mar_rank[2], length=rank_mid+2)[2:(rank_mid+1)])
	##print(mid_ranks)
	#states_plot <- rank_order[[ranked_by]][c(length(rank_order[[ranked_by]]):(length(rank_order[[ranked_by]])-mar_rank[1]+1), mid_ranks, mar_rank[2]:1)]
	#print(states_plot)
	...
	states_plot_rank <- rev(c(1:mar_rank[1], mid_ranks, (length(rank_order[[ranked_by]])-mar_rank[2]+1):length(rank_order[[ranked_by]])))
	states_plot_rank_print <- c(length(rank_order[[ranked_by]]):(length(rank_order[[ranked_by]])-mar_rank[1]+1), rev(mid_ranks), mar_rank[2]:1)
	#print(states_plot_rank_print)
	...
	for(i in 1:(length(legend_labels))){
		...
		if(i < length(legend_labels) && states_plot_rank[i] - states_plot_rank[i+1] > 1){
	...
	# Remove any states with D or N grade
	#order_plot <- order_plot[order_plot %in% names(state_grades[!state_grades %in% c('D', 'N')])]
	...
	point_col <- setNames(rep(NA, nrow(testing_stats)), rownames(testing_stats))
	...
	text(x=2.5*x_range[1], y=3.5, labels=toupper('Lower testing rate\nLower % positive'), col=bg_txt_col, cex=bg_text_cex)
	text(x=0.41, y=32, labels=toupper('Higher testing rate\nHigher % positive'), col=bg_txt_col, cex=bg_text_cex)
	text(x=2.5*x_range[1], y=32, labels=toupper('Lower testing rate\nHigher % positive'), col=bg_txt_col, cex=bg_text_cex)
	text(x=0.41, y=1.8, labels=toupper('Higher testing rate\nLower % positive'), col=bg_txt_col, cex=bg_text_cex)

	# Add horizontal background line and label
	h_at <- 5
	abline(h=h_at, lty=2, col=bg_txt_col, lwd=2)
	text(x=x_range[1], y=h_at, labels=paste0(h_at, '% positive'), cex=1.1*bg_text_cex, xpd=TRUE, adj=c(0.2,-0.5), col=bg_txt_col)

	# Add vertical background line and label
	v_at <- 0.2
	abline(v=v_at, lty=2, col=bg_txt_col, lwd=2)
	text(x=v_at, y=y_range[2], labels=paste0(v_at, '% tested'), cex=1.1*bg_text_cex, xpd=TRUE, adj=c(0.85,-0.5), srt=90, col=bg_txt_col)
	
	# Point color legend
	legend(x=x_range[1], y=y_range[1], legend=paste0('Grade ', names(grade_cols), ' reporting'), col=grade_cols, xjust=0.05, yjust=0.2, bty='n', pch=15)
	
	# Set line label length
	line_lab_len <- 0.06*c(1, diff(y_range) / diff(x_range))
	...
	for(state in rownames(testing_stats)){
		...
		if(state %in% names(labels_out)){
			...
			if(labels_out[state] == 'upper right') text_adj <- c(-0.2,-0.2)
			...
			# Draw line
			segments(x0=xy[1], y0=xy[2], x1=line_end[1], y1=line_end[2], col=point_col[state])

			# Add text labels inside circle
			text(x=line_end[1], y=line_end[2], labels=state, cex=txt_cex, adj=text_adj, col=point_col[state])
	...
	text(x=x_range[2] - 0.5*diff(x_range), y=y_range[1] + 0.05*diff(y_range), labels=toupper('Higher testing rate\nLower % positive\nSlower inc in % positive'), col=bg_txt_col, cex=bg_text_cex)
	text(x=x_range[1] + 0.03*diff(x_range), y=y_range[2] - 0.05*diff(y_range), labels=toupper('Lower testing rate\nHigher % positive\nFaster inc in % positive'), col=bg_txt_col, cex=bg_text_cex)

	# Point color legend
	legend(x=x_range[1], y=y_range[1], legend=paste0('Grade ', names(grade_cols), ' reporting'), col=grade_cols, xjust=0.05, yjust=0.2, bty='n', pch=15)

	# Set line label length
	line_lab_len <- 0.7*c(10, 0.01)
