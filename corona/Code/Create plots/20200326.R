...
run <- function(){
	...
	mar_rank <- c(5,6)		# Number of states to include at slowest and fastest of ranking
	rank_mid <- 4			# Number of middle ranking states to show
	...
	start_cases <- 15		# Number of cases when to align curves
	...
	read_grades <- read.csv('../../Data/us_state_grades.csv')
...
#		'intercept'=rownames(log_sfp_lm_mat_nna)[order(log_sfp_lm_mat_nna[, 'intercept'])],
	...
	line_lab_shift <- 0.01*c(diff(x_range), diff(y_range))
	...
	text(x=2.5*x_range[1], y=3, labels=toupper('Lower testing rate\nLower % positive'), col=bg_txt_col, cex=bg_text_cex)
	text(x=0.26, y=48, labels=toupper('Higher testing rate\nHigher % positive'), col=bg_txt_col, cex=bg_text_cex)
	text(x=2.5*x_range[1], y=28, labels=toupper('Lower testing rate\nHigher % positive'), col=bg_txt_col, cex=bg_text_cex)
	text(x=0.26, y=1.55, labels=toupper('Higher testing rate\nLower % positive'), col=bg_txt_col, cex=bg_text_cex)
